...
export class EmailService {
  ...
  async sendLoginToken(to: string, token: string, name: string) {
    this.logger.log(`Enviando token de login para ${to}`);
    try {
      ...
      this.logger.log(`Token de login enviado com sucesso para ${to}`);
    } catch (err) {
      this.logger.error(`Erro ao enviar email de token para ${to}: ${err.message}`, err.stack);
      throw err;
    }
  }
}
