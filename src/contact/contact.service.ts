...
export class ContactService {
  ...
  async create(botId: string, userId: string, dto: CreateContactDto) {
    ...
    if (existing) throw new ConflictException('Contato com esse número já existe para este bot');
  ...
  async findOne(botId: string, userId: string, id: string) {
    await this.ensureBotOwnership(botId, userId);
    const contact = await this.prisma.contact.findFirst({ where: { id, botId } });
    if (!contact) throw new NotFoundException('Contato não encontrado');
    return contact;
  }

  async update(botId: string, userId: string, id: string, dto: UpdateContactDto) {
    await this.findOne(botId, userId, id);
    if (dto.phone) {
      const conflict = await this.prisma.contact.findFirst({
        where: { botId, phone: dto.phone, NOT: { id } },
      });
      if (conflict) throw new ConflictException('Número já pertence a outro contato');
    }
